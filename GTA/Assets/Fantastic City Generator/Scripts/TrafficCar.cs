...
namespace FCG
{
    public class TrafficCar : MonoBehaviour
    {
        ...
        //[HideInInspector]
        public float distanceToSelfDestroy = 0; //0 = Do not autodestroy with camera distance
...
/*void SelfDestructWhenAwayFromThePlayer()
        {
            ...
            if (speed < 2 && status != StatusCar.stoppedAtTrafficLights && (Time.time > timeStoped + 30) & InTheFieldOfVision(transform.position, camera))
            {
            ...
            else if (Vector3.Distance(transform.position, camera.position) < distanceToSelfDestroy || InTheFieldOfVision(transform.position, camera))
                countC = 0;
            else
            {
                countC++;
      ...
      /*  public void SelfDestructWhenAwayFromThePlayerInit()
        {
            if (tSystem && camera)
                if (Vector3.Distance(transform.position, camera.position) > distanceToSelfDestroy && !InTheFieldOfVision(transform.position, camera))
                {
                    tSystem.nVehicles--;
                    Destroy(this.gameObject);
                }
        ...
        bool InTheFieldOfVision(Vector3 source, Transform target)
        {
            ...
            // the IACar wants to disappear without it being seen by the camera/camera
