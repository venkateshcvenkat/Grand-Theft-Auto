...
public class RCC_Editor : Editor {
    ...
    private void InputsTab() {
        ...
        EditorGUI.indentLevel++;
        EditorGUILayout.HelpBox("Vehicle will be controllable with external inputs. Vehicle won't act as a camera vehicle, won't receive camera inputs from the ''RCC_InputManager''. Used on AI vehicles.", MessageType.Info);
        EditorGUI.indentLevel--;

        EditorGUILayout.Space();
        EditorGUILayout.PropertyField(serializedObject.FindProperty("overrideInputs"), new GUIContent("Override Inputs", "Override vehicle's inputs with given inputs. Vehicle won't receive camera inputs while this option is enabled. You can use OverrideInputs method to feed this vehicle with your own inputs."), false);

        EditorGUI.indentLevel++;
        EditorGUILayout.HelpBox("Vehicle will be controllable with given inputs. Vehicle won't receive camera inputs from the ''RCC_InputManager'' while this option is enabled. You can use ''OverrideInputs'' method to feed this vehicle with your own inputs. Vehicle will act as camera vehicle if externalController is disabled.", MessageType.Info);
        EditorGUI.indentLevel--;
